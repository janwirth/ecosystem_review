...
| **Configuration-file fallback** (TOML / YAML / JSON args from a config file) | Defaults from `~/.config/<tool>.toml` | Use [`tom`](https://hex.pm/packages/tom) (TOML) or [`gleam_yaml`](https://hex.pm/packages/gleam_yaml) (YAML); merge into your typed config record before parsing argv. |
