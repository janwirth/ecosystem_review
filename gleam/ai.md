...
- **Prompt management** — no template engine designed for LLM prompts (Jinja-style, with versioning). Use string interpolation or [trick](parsers-and-generators/generate.md#trick) for typed templates.
...
- **No serialization / generator overlap with [parsers-and-generators](parsers-and-generators/README.md):** [toon_codec](#toon_codec) is technically a serialization format, but its purpose (LLM input optimization) and category land it squarely here.
