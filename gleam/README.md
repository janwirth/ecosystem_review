...
- [YAML](yaml.md) — pure-Gleam dual-target parsers (`taffy` YAML 1.2 + alias-bomb hardening, `yamleam` decoder-API), `yamerl`-FFI parsers (`yay` dual + typed errors, `glaml` BEAM-only stale), the only emitter (`cymbal` by Louis Pilfold), domain-shaped (`mb_rep_gleam` for Metabase Representation Format), cross-link to [`yodel`](env-vars.md#yodel) for `${VAR}`-interpolated config. Background on YAML 1.1 vs 1.2, alias bombs, anchors / tags / merge keys. Decision table + FFI escape hatches (`yamerl`, `fast_yaml`). **Note:** the Hex package literally named `gleam_yaml` is a stale 2024 v0.0.1 prototype — disregarded. 6 packages reviewed (+ 4 disregarded).
