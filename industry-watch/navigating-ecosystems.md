...
This repo — [ecosystem-review](README.md) — is **one** approach to library evaluation: snapshot-dated, public-web-UI-only, scored on the [same 7-dim rubric](../practices/formalization.md#scoring-dimensions-per-repo) every time. That approach is opinionated. It's slow, manual, and trades coverage for reproducibility. There are many other ways to do this work, and most readers will mix several of them.
...
- **GitHub-only.** GitLab, Codeberg, Sourcehut, Forgejo, Bitbucket projects are invisible. For Gleam this is mostly fine; for some C/Linux ecosystem projects (Graphviz on GitLab — see [diagramming review](../design/diagramming.md#graphviz--dot)) it materially undercounts.
...
The repo's [main 7-dim rubric](../practices/formalization.md#scoring-dimensions-per-repo) doesn't translate cleanly to discovery tools (you can't measure README maturity of a SaaS site the same way as a library). Here's a discovery-tool-specific scaffold.
...
> [!IMPORTANT]
> The structured review (step 9) is the differentiator of this repo's approach. Awesome lists, ossinsight, deps.dev, and friends are *inputs* — they tell you what's true *now*. The structured review captures *why we decided what we decided* in a way that survives the inputs going stale. See [formalization.md](../practices/formalization.md) for the dimensions and [workflows/REVIEW_METHODOLOGY.md](../workflows/REVIEW_METHODOLOGY.md) for the process.
...
- [Diagramming tools cross-ecosystem survey](../design/diagramming.md) — broad cross-language survey using the same rubric.
- [Mobile apps cross-ecosystem](../application-types/mobile.md) — comparing 14 mobile frameworks side-by-side.
- [Recent security incidents](recent-incidents.md) — companion article showing what happens when the discovery filter misses supply-chain risk.
- [Programming language popularity](language-popularity.md) — Stack Overflow survey data as a meta-popularity signal.
- [Formalization](../practices/formalization.md) — the scoring rubric and discovery pipeline used across all reviews.
